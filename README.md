# Header management without headaches

`remedy` is an opinionated license header updater (see the "Approach" section below).
It allows to consistently insert and update license headers in source files or even change licenses completely!

## Quick start

By default, `remedy` looks for a configuration file named `remedy.json` in the directory in which it is invoked:
...
```shell
 cd $(mktemp -d) && go get -u github.com/elzorrorebelde/remedy/cmd/remedy && cd -
 ${GOBIN:-`go env GOPATH`/bin}/remedy
```

As a result, source files will be changed and `.remedy-run` will be generated to keep track of `remedy` last execution.
...
```shell
 go get -u github.com/elzorrorebelde/remedy
 ${GOBIN:-`go env GOPATH`/bin}/remedy --configuration /path/to/configuration.json
```
...
`remedy` approach to copyright is well explained in [this stackoverflow answer](https://stackoverflow.com/a/2391555/277128),
read it first!
...
Based on this premise, `remedy` will process all files matching the configuration and that have been changed since its last execution.
`remedy` will then compute the copyright year, file by file, from their available versioning information (typically by retrieving the relevant dates from Git commits).
...
`remedy` will never overwrite the start date of the copyright year if it finds one, if and only if that date occurs earlier than the first commit date of the file.

### Configuration

`remedy` relies on the emerging [JSON Schema standard](https://json-schema.org/) to validate its configuration.
`remedy` schema is defined [here](https://raw.githubusercontent.com/elzorrorebelde/remedy/master/docs/schema.json).
...
| `style`          | string                  | **[required]** See all the possible names [here](https://raw.githubusercontent.com/elzorrorebelde/remedy/master/docs/schema.json). The lookup is case-insensitive. |
